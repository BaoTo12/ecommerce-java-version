...
import com.ecommerce.monolith.domain.inventory.entity.InventoryEntity;
...
public class InventoryService {
   ...
   */
  @Transactional
  public void reserveForOrder(OrderEntity order) {
    ...
    for (OrderItemEntity item : order.getItems()) {
      ...
      if (updated == 1) {
        ...
        reservationRepo.save(
            InventoryReservationEntity.reserved(orderId, productId, quantity, expiresAt));
  ...
  private boolean reserveWithOptimisticRetry(
    ...
    reservationRepo.save(
        InventoryReservationEntity.failed(orderId, productId, qty, "LOCK_EXHAUSTED"));
    return false;
  }
  ...
  public boolean doOptimisticReserve(UUID orderId, UUID productId, int qty, Instant expiresAt) {
    ...
    if (!inv.hasSufficientStock(qty)) {
      reservationRepo.save(
          InventoryReservationEntity.failed(orderId, productId, qty, "INSUFFICIENT_STOCK"));
      return false;
    }

    inv.reserve(qty);
    inventoryRepo.save(inv); // @Version check happens here
    reservationRepo.save(InventoryReservationEntity.reserved(orderId, productId, qty, expiresAt));
    return true;
  }

  /** Release reserved stock (compensation transaction). */
  @Transactional
  public void releaseForOrder(UUID orderId) {
    List<InventoryReservationEntity> reservations = reservationRepo.findByOrderId(orderId);
    for (InventoryReservationEntity res : reservations) {
      if ("RESERVE".equals(res.getOperation())
          && "SUCCESS".equals(res.getStatus())
          && res.getReleasedAt() == null) {
        inventoryRepo.atomicIncrement(res.getProductId(), res.getQuantity());
        res.markReleased();
        reservationRepo.save(res);
        log.info(
            "Released: product={}, qty={}, order={}",
            res.getProductId(),
            res.getQuantity(),
            orderId);
      }
   ...
   */
  @Transactional
  public void releaseExpiredReservation(InventoryReservationEntity res) {
    inventoryRepo.atomicIncrement(res.getProductId(), res.getQuantity());
    res.markReleased();
    reservationRepo.save(res);
    log.info(
        "TTL expiry released: product={}, qty={}, order={}",
        res.getProductId(),
        res.getQuantity(),
        res.getOrderId());
  }
