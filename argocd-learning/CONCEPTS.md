# ArgoCD Core Concepts — Explained

---

## Concept 1: What are Application, Project, and AppProject?

### Application
An ArgoCD Application defines:
- **Source:** Where the desired state lives (Git repo, path, branch)
- **Destination:** Where to deploy (Kubernetes cluster, namespace)
- **Sync Policy:** How to detect and apply differences

### Project (AppProject)
A Project groups related Applications and sets boundaries:
- Which Git repos are allowed as sources?
- Which namespaces can be deployed to?
- Which Kubernetes resource types are allowed?

Default project allows everything — fine for learning, use named projects for production teams.

```yaml
# Example: restrict payment-team to only their namespace
spec:
  sourceRepos: ['https://github.com/BaoTo12/ecommerce-java-version.git']
  destinations:
    - namespace: payment
...
## Concept 2: Automated vs Manual Sync

### Manual Sync (default)
ArgoCD DETECTS the diff but does NOT apply it automatically.
A human must click "Sync" in the UI or run `argocd app sync`.

When to use: Terraform-managed resources, databases, anything risky.
...
syncPolicy:
  automated:
    ...
    selfHeal: true   # Re-apply if someone manually changed k8s state
```
When to use: Application deployments (our microservices).

**What is `selfHeal`?**
Without selfHeal: If someone does `kubectl scale deployment order-service --replicas=0`,
ArgoCD detects it but doesn't fix it.
With selfHeal: ArgoCD auto-reverts to what's in git (desired state wins).

**What is `prune`?**
Without prune: If you delete a Deployment from git, ArgoCD won't delete it from k8s.
With prune: Deleting from git → ArgoCD deletes from k8s.
