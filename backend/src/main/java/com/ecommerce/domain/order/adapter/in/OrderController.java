package com.ecommerce.domain.order.adapter.in;
...
public class OrderController {
    ...
    public ResponseEntity<CheckoutSessionResponse> checkoutSession(
        ...
        return ResponseEntity.ok(orderUseCase.checkoutSession(request));
    }
    ...
    public ResponseEntity<String> executePayment(
        ...
        String responseBody = orderUseCase.checkout(idempotencyKey, request);
    ...
    public Page<OrderResponse> listOrders(@PageableDefault(size = 20) Pageable pageable) {
        return orderUseCase.listOrders(pageable);
    }
